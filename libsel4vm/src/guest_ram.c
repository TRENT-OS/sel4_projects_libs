...
static int vm_ram_register_helper(vm_t *vm, vm_memory_reservation_t *res,
                                  ...
                                  void *cookie)
{
    ...
    err = map_vm_memory_reservation(vm, res, map_iterator, cookie);
    if (err) {
        ZF_LOGE("Failed to map new ram reservation");
        vm_free_reserved_memory(vm, res);
        return -1;
    }
...
uintptr_t vm_ram_register(vm_t *vm, size_t bytes)
{
    ...
    vm_memory_reservation_t *res = vm_reserve_anon_memory(vm, bytes, BIT(seL4_PageBits),
                                                          default_ram_fault_callback,
                                                          NULL, &base_addr);
    if (!res) {
    ...
    /* We map the reservation immediately, by-passing the deferred mapping
     * functionality. This allows us the allocate, touch and manipulate VM RAM
     * prior to the region needing to be faulted upon first.
     * This calls vm_free_reserved_memory() in error
     */
    int err = vm_ram_register_helper(vm, res, base_addr, bytes,
                                     ram_alloc_iterator, vm);
    if (err) {
        ZF_LOGE("Failed to register RAM");
        return 0;
    }
...
int vm_ram_register_at_custom_iterator(vm_t *vm, uintptr_t start, size_t bytes,
                                       ...
                                       void *cookie)
{
    vm_memory_reservation_t *res = vm_reserve_memory_at(vm, start, bytes,
                                                        default_ram_fault_callback,
                                                        NULL);
    if (!res) {
        ZF_LOGE("Unable to reserve RAM at addr 0x%"PRIxPTR" of size 0x%zx",
                start, bytes);
        return -1;
    }
     ...
     * This calls vm_free_reserved_memory() in error
     */
    return vm_ram_register_helper(vm, res, start, bytes, map_iterator, cookie);
}

int vm_ram_register_at(vm_t *vm, uintptr_t start, size_t bytes, bool untyped)
{
    memory_map_iterator_fn iterator = untyped ? ram_ut_alloc_iterator
                                      : ram_alloc_iterator;
    return vm_ram_register_at_custom_iterator(vm, start, bytes, iterator, vm);
}
